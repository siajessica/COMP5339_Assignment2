...
def FuelStationIntegration(access_token, API_KEY_FUEL, get_new_price=True, batch_size = 2000):
    """
    ...
    merged_batches = []
    for i in range(0, len(df_prices_api), batch_size):
        batch = df_prices_api.iloc[i:i+batch_size]
        merged = batch.merge(df_station_api, left_on="stationcode", right_index=True, how="inner")
